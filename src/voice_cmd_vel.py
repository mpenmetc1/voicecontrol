...
class voice_cmd_vel:
    ...
    def speechCb(self, twist_msg, bool_msg):
        rospy.loginfo(twist_msg.data)

        if twist_msg.data.find("full speed") > -1:
            if self.speed == 0.2:
                self.twist_msg.linear.x = self.twist_msg.linear.x*2
                self.twist_msg.angular.z = self.twist_msg.angular.z*2
                self.speed = 0.4
        if twist_msg.data.find("half speed") > -1:
            if self.speed == 0.4:
                self.twist_msg.linear.x = self.twist_msg.linear.x/2
                self.twist_msg.angular.z = self.twist_msg.angular.z/2
                self.speed = 0.2

        if twist_msg.data.find("forward") > -1:    
            self.twist_msg.linear.x = self.speed
            self.twist_msg.angular.z = 0
        elif twist_msg.data.find("left") > -1:
            if self.twist_msg.linear.x != 0:
                if self.twist_msg.angular.z < self.speed:
                    self.twist_msg.angular.z += 0.05
            else:        
                self.twist_msg.angular.z = self.speed*2
        elif twist_msg.data.find("right") > -1:    
            if self.twist_msg.linear.x != 0:
                if self.twist_msg.angular.z > -self.speed:
                    self.twist_msg.angular.z -= 0.05
            else:        
                self.twist_msg.angular.z = -self.speed*2
        elif twist_msg.data.find("back") > -1:
            self.twist_msg.linear.x = -self.speed
            self.twist_msg.angular.z = 0
        elif twist_msg.data.find("stop") > -1 or twist_msg.data.find("halt") > -1:          
            self.twist_msg = Twist()
